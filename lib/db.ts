...
export async function seedDatabase() {
  try {
    ...
    // 2. Seed default site content (CMS editable fields) — runs independently
    const existingContent = await db.select().from(schema.siteContent).limit(1);
    if (existingContent.length === 0) {
      ...
      await db.insert(schema.siteContent).values([
        ...
        { id: 'sc-home-hero-image', key: 'home_hero_image', value: '/professional-carpet-cleaning-service-in-modern-hom.jpg', label: 'Hero Image Path', group: 'home', type: 'text' },
