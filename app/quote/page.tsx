...
function DateTimePicker({
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4">
      <div className="flex-1 min-w-0">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" id="date-picker" className={`w-full mt-1.5 justify-between font-body text-sm h-11 rounded-lg border-border bg-background ${isSelectedDateClosed ? 'text-destructive' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <Label htmlFor="time-picker" className="text-xs font-heading text-foreground">Time</Label>
        {date && !isSelectedDateClosed ? (
          <div className="relative">
            <select id="time-picker" value={time || ''} onChange={(e) => onTimeChange(e.target.value)}
              className="w-full mt-1.5 px-3 py-2.5 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary h-11 appearance-none font-body">
              <option value="">Select a time slot</option>
              {availableSlots.map((slot) => (<option key={slot} value={slot}>{slot}</option>))}
            </select>
            <ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none mt-0.5" />
            {availableSlots.length === 0 && <p className="text-xs text-amber-500 font-body mt-1">No available time slots for this date.</p>}
          </div>
        ) : (
          <div className="w-full mt-1.5 px-3 py-2.5 text-sm border border-border rounded-lg bg-muted/30 h-11 flex items-center text-muted-foreground font-body">
      ...
      <div className="min-h-screen bg-background py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-center mb-10">
            {[1, 2].map((step) => (
              <div key={step} className="flex items-center">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 font-heading-bold text-sm transition-all ${currentStep >= step ? "bg-primary border-primary text-primary-foreground" : "bg-white border-border text-muted-foreground"}`}>{step}</div>
                {step < 2 && <div className={`w-16 md:w-24 h-0.5 mx-2 transition-all ${currentStep > step ? "bg-primary" : "bg-border"}`} />}
              </div>
            ))}
          </div>

          {/* Form */}
          <div className="bg-white rounded-xl border border-border shadow-sm p-8 md:p-10">
            {currentStep === 1 && (
              <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {activeServices.map((service) => (
                      <button key={service.id} onClick={() => toggleService(service.id)}
                        className={`p-4 rounded-xl border-2 text-left transition-all relative ${formData.services.includes(service.id) ? "border-primary bg-primary/5" : "border-border hover:border-primary/50 bg-white"}`}>
                        <div className="flex items-center gap-3"><span className="text-2xl">{service.icon}</span><span className="text-sm font-heading text-foreground">{service.name}</span></div>
                        {formData.services.includes(service.id) && <div className="absolute top-2 right-2 w-5 h-5 bg-primary rounded-full flex items-center justify-center"><Check className="w-3 h-3 text-white" /></div>}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between">
                  ...
                  <Button onClick={handleNext} className="rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-nav text-sm px-7">Continue <ArrowRight className="ml-2 h-4 w-4" /></Button>
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between">
                  <Button onClick={handleBack} variant="outline" className="rounded-full border-border font-nav text-sm"><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button asChild variant="outline" className="rounded-full border-border font-nav text-sm"><Link href="tel:0430799567"><Phone className="mr-2 h-4 w-4" /> Quick Call Quote</Link></Button>
                    <Button onClick={handleSubmit} className="rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-nav text-sm px-7">Submit Quote Request</Button>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="mt-6 text-center text-sm text-muted-foreground font-body">
            Need immediate assistance? <Link href="tel:0430799567" className="text-primary font-heading hover:underline">Call us at 0430 799 567</Link>
          </div>
        </div>
      </div>
      <Footer />
